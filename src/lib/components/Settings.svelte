<script lang="ts">
  ...
  const languagesFirstLanguage = languages.filter(lang => 
    lang.value === "english" || lang.value === "spanish"
  );
  ...
  $: selectedValue2 =
    languages.find((f) => f.value === value2)?.label ?? "Select a language...";
  ...
  function updatePreferences() {
    preferences.update(p => ({
      ...p,
      firstLanguage: languagesFirstLanguage.find(f => f.value === value)?.label || p.firstLanguage,
...
<Dialog.Root>
  ...
  <Dialog.Content>
   <Dialog.Header>
    ...
    <Dialog.Description>
     Augment your Language Preferences Here!
    </Dialog.Description>
   </Dialog.Header>
               ...
               {#each languages as language}
                 ...
                 >
                   <Check
                     class={cn(
                       "mr-2 h-4 w-4",
                       value2 !== language.value && "text-transparent"
                     )}
                   />
                   {language.label}
                 </Command.Item>
